...
(function(){
  ...
  const achievementsEl = document.getElementById("pm-achievements");
  ...
  function openModal(card){
    ...
    const desc = card.querySelector(".pc-top ~ p")?.textContent.trim() || card.querySelector("p:not(.pc-featured)")?.textContent.trim() || "";
    ...
    slugEl.textContent = "~/" + (slugify(title) || "project");
    titleEl.textContent = title;

    tagsEl.innerHTML = "";
    tags.forEach(t=>tagsEl.appendChild(t.cloneNode(true)));
    if(meta) tagsEl.appendChild(meta.cloneNode(true));

    overviewEl.innerHTML = "";
    achievementsEl.innerHTML = "";

    if(detail){
      const detailOverview = detail.querySelector(".pm-detail-overview");
      const detailAchievements = detail.querySelector(".pm-detail-achievements");
      if(detailOverview) overviewEl.innerHTML = detailOverview.innerHTML;
      if(detailAchievements){
        Array.from(detailAchievements.children).forEach(li=>{
          const clone = document.createElement("li");
          clone.textContent = li.textContent;
          achievementsEl.appendChild(clone);
        });
      }
    } else {
      ...
      splitSentences(desc).forEach(sentence=>{
    ...
    stackEl.innerHTML = "";
    chips.forEach(c=>stackEl.appendChild(c.cloneNode(true)));
